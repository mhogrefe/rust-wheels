...
use rust_wheels::iterators::dependent_pairs::exhaustive_dependent_pairs_infinite;
...
fn main() {
    let fr = &f;
    for p in exhaustive_dependent_pairs_infinite(exhaustive_u::<u64>(), fr).take(10000) {
        println!("{:?}", p);
    }
    if 1 == 1 {
        return;
